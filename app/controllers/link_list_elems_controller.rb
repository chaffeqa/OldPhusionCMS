class LinkListElemsController < ApplicationController
  ...
  def create
    ...
    if @link_list_elem.save and @element.save and @node.template.elements << @element
      redirect_to link_list_elem_path(@link_list_elem)
    else
      render :action => 'new'
    end
  end
  ...
  def update
    ...
    if @link_list_elem.update_attributes(params[:link_list_elem])
      redirect_to @link_list_elem, :notice => "Link List Element successfully updated."
    else
      render :action => 'edit'
    end
  end
