class LinksController < ApplicationController
  ...
  def create
    ...
    if @link.save and @link_list_elem.links << @link
      redirect_to(@link_list_elem, :notice => 'Link was successfully added.')
    else
      render :action => "new" 
    end
  end
  ...
  def update
    ...
    if @link.update_attributes(params[:link])
      redirect_to(@link_list_elem, :notice => 'Link was successfully updated.')
    else
      render :action => "edit"
    end
  end
  ...
  def destroy
    @link = Link.find(params[:id])
    @link.destroy
    redirect_to(link_list_elem_url(@link_list_elem), :notice => 'Link was successfully destroyed.')
  end
